...
def run_multi_domain_crawler(base_url: str, crawler_config: Dict[str, Any], output_manager: OutputManager) -> subprocess.Popen:
    ...
    try:
        process = subprocess.Popen(
            cmd,
            cwd=os.path.abspath(os.path.join(os.path.dirname(__file__), "../src/multi_domain_crawler")),
...
async def run_crawler(base_url: str, domain_config: Dict[str, Any], output_manager: OutputManager) -> bool:
    """Run crawler component with standardized path management and improved error handling."""
    ...
    logger.info(f"Crawler output directory: {output_dir}")
    logger.info(f"Crawler state file: {state_file}")
    logger.info(f"Crawler log file: {log_file}")
    
    # Ensure directories exist
    output_manager.ensure_path_exists("crawler")
    output_manager.ensure_path_exists("logs")
    logger.info("Verified output directories exist")
    
    try:
        # Prepare the domain parameter correctly
        # Estrai il dominio base senza path
        clean_domain = output_manager.domain.replace("http://", "").replace("https://", "").replace("www.", "")
        clean_domain = clean_domain.split('/')[0]
        ...
        logger.info(f"Max URLs: {crawler_config.get('max_urls', 1000)}")
        logger.info(f"Hybrid mode: {crawler_config.get('hybrid_mode', True)}")
        logger.info(f"Request delay: {crawler_config.get('request_delay', 0.25)}")
        
        # Start the multi-domain crawler as a subprocess
        try:
            # Double-check the process doesn't already exist
            if base_url in crawler_processes and crawler_processes[base_url] and crawler_processes[base_url].poll() is None:
                logger.warning(f"Crawler process for {base_url} is already running")
                return False
                
            # Set up the crawler command explicitly
            # Prepare command
            cmd = [
                "python", "-m", "scrapy", "crawl", "multi_domain_spider",
                ...
                "-s", f"LOG_LEVEL=DEBUG",  # Use DEBUG for more details
                "-s", f"PIPELINE_REPORT_FORMAT=all",
                "--logfile", f"{log_file}"
            ]
            ...
            logger.info(f"Crawler working directory: {cwd}")
            if not os.path.exists(cwd):
                logger.error(f"Crawler working directory not found: {cwd}")
                # Try alternate path
                cwd = os.path.abspath(os.path.join(os.path.dirname(__file__), "../multi_domain_crawler"))
                logger.info(f"Trying alternate working directory: {cwd}")
                if not os.path.exists(cwd):
                    logger.error(f"Alternate working directory not found: {cwd}")
                    return False
            ...
            if process.pid is None:
                logger.error(f"Failed to start crawler process for {base_url}")
                return False
                
            logger.info(f"Crawler started for {base_url} with PID {process.pid}")
            crawler_processes[base_url] = process
        except Exception as e:
            logger.exception(f"Error starting crawler process: {e}")
            return False
